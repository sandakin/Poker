...
//    ♠
    ...
    public cards (String s) {
        ...
        int rank = -1;
        for (int i = 0; i < cards.NO_OF_RANKS; i++) {
        ...
        int suit = -1;
        for (int i = 0; i < cards.NO_OF_SUITS; i++) {
...
//    }
    ...
    public String rethashCode(int a) {
    	
    	for(int i=0;i<4;i++)
    	{
    	for(int j=0;j<13;j++)
    	{
		    	if(((j * NO_OF_SUITS) + i)==a){
		    		return RANK_SYMBOLS[j] + SUIT_SYMBOLS[i];
		    	}
    ...
    public boolean equals(Object obj) {
        if (obj instanceof cards) {
            return ((cards) obj).hashCode() == hashCode();
        } else {
            return false;
        }
    }
